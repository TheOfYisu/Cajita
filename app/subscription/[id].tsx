...
export default function SubscriptionEditor() {
  ...
  const initialKind: RecurringKind =
    existing?.kind ??
    (RECURRING_KINDS.some((k) => k.key === params.kind)
      ? (params.kind as RecurringKind)
      : "subscription");
  ...
  const [variableAmount, setVariableAmount] = React.useState<boolean>(
    existing?.variableAmount ?? true,
  );
  const [dueDay, setDueDay] = React.useState<number>(existing?.dueDay ?? 5);
  const [frequency, setFrequency] = React.useState<Recurring["frequency"]>(
    existing?.frequency ?? "monthly",
  );
  const [firstDate, setFirstDate] = React.useState<number>(
    existing?.nextRun ?? nextMonthGuess(),
  );
  const [accountId, setAccountId] = React.useState<number | null>(
    existing?.accountId ?? accounts[0]?.id ?? null,
  );
  ...
  const [leadDays, setLeadDays] = React.useState<number>(
    existing?.leadDays ?? prefs.defaultLeadDays,
  );
  const [notify, setNotify] = React.useState<boolean>(
    existing ? existing.notify : true,
  );

  if (!isNew && !existing) {
    return (
      <ModalScreen title="Recurrente">
  ...
  const selCats = categories.filter((c) =>
  ...
  const save = async () => {
    ...
    if (!title.trim()) {
      Alert.alert(`Ponle un nombre al ${noun}`);
      return;
    }
    if (!amt || amt <= 0) {
      Alert.alert(checklist ? "Ingresa un monto estimado" : "Monto inválido");
      return;
    }
    if (!accountId) {
      Alert.alert("Selecciona la cuenta");
      return;
    }
    ...
    >
      ...
      <Card style={{ marginBottom: 12 }}>
        <Field
          ...
          placeholder={
            checklist
              ? "Ej: Agua, Luz, Internet, Arriendo"
              : "Ej: Netflix, Gimnasio Smart Fit"
          }
          autoFocus
        />
        <Field
      ...
      {checklist ? (
        <>
          ...
          <Text
            style={{ color: colors.textMuted, fontSize: 12, marginBottom: 14 }}
          >
        ...
        {notify ? (
          <View style={{ paddingHorizontal: 8, paddingBottom: 8 }}>
            ...
            {preview ? (
              ...
              >
                Recibirás avisos {leadDays} día{leadDays > 1 ? "s" : ""} antes,
                el día anterior y el mismo día (
                {new Date(preview).toLocaleDateString("es-CO", {
                  day: "2-digit",
                  month: "short",
                })}
                ).
      ...
      <Card style={{ padding: 8 }}>
        ...
        >
          Si divides este {noun} con otra persona, asóciala aquí. Se mostrará su
          correo en los recordatorios.
        </Text>
        <PickRow
          label="Solo yo"
          icon="person-outline"
      ...
      {person ? (
        <Text style={{ color: colors.textMuted, fontSize: 12, marginTop: 10 }}>
          {formatMoney(
            parseFloat(amount.replace(/[^\d.]/g, "")) / 2 || 0,
            prefs.currency,
          )}{" "}
          por persona si se divide en partes iguales.
        </Text>
      ) : null}
      <View style={{ height: 8 }} />
      <Text style={{ color: colors.textMuted, fontSize: 11 }}>
        {existing && existing.nextRun && !checklist
          ? `Próximo cobro en ${daysUntil(existing.nextRun)} días`
          : ""}
      </Text>
    </ModalScreen>
  );
