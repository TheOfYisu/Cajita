...
export default function HomeScreen() {
  ...
  const fixedPending = fixed.items.filter(
    (it) => it.status === "pending" || it.status === "overdue",
  );
  const fixedDone = fixed.items.filter(
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={[styles.netCard, { backgroundColor: colors.accent }]}>
          ...
          <View style={styles.netRow}>
            <View style={styles.netItem}>
              ...
              <Text style={styles.netItemValue}>
                {formatMoney(totalAssets, prefs.currency, hide)}
              </Text>
            </View>
            ...
            <View style={styles.netItem}>
              ...
              <Text style={styles.netItemValue}>
                {formatMoney(totalDebt, prefs.currency, hide)}
              </Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.quickRow}>
          <Quick
            ...
            onPress={() => router.push("/transaction/new?mode=expense")}
          />
          <Quick
            icon="arrow-up"
            label="Ingreso"
            color={colors.positive}
            onPress={() => router.push("/transaction/new?mode=income")}
          />
          <Quick
          ...
          <>
            ...
            >
              ...
              >
                <View style={{ flex: 1 }}>
                  ...
                  >
                    ...
                    {fixed.pendingTotal > 0
                      ? ` · faltan ${formatMoney(fixed.pendingTotal, prefs.currency, hide)}`
                      : ""}
                  </Text>
                </View>
                <Ionicons
              ...
              <View
                style={{
                  marginTop: 10,
                  marginBottom: fixedPending.length ? 12 : 0,
                }}
              >
                <ProgressBar
                  pct={fixed.items.length ? fixedDone / fixed.items.length : 0}
                  color={colors.positive}
                />
              </View>
              {fixedPending.slice(0, 3).map((it) => (
                <View
                  key={it.payment.id}
                  style={[styles.fixedRow, { borderColor: colors.border }]}
                >
                  <View style={{ flex: 1 }}>
                    <Text
                      style={{
                    ...
                    >
                      {it.status === "overdue" ? "Vencido" : "Vence"}{" "}
                      {new Date(it.payment.dueDate).toLocaleDateString(
                        "es-CO",
                        { day: "2-digit", month: "short" },
                      )}
                      {" · "}
                      {it.recurring.variableAmount ? "~" : ""}
                      {formatMoney(
                        Math.abs(it.recurring.amount),
                        prefs.currency,
                        hide,
                      )}
                    </Text>
          ...
          >
            ...
            {cash.active.map((w) => {
              ...
              return (
                <Pressable
                  key={w.id}
                  onPress={() => router.push(`/cash/${w.id}` as never)}
                  style={{ marginTop: 10 }}
                >
                  <View
                    style={{
                  ...
                  >
                    <Text style={{ color: colors.textMuted, fontSize: 12 }}>
                      {w.note || "Retiro"} · retiraste{" "}
                      {formatMoney(w.amount, prefs.currency, hide)}
                    </Text>
                    <Text
                      style={{
                        color: w.remaining < 0 ? colors.negative : colors.text,
                        fontWeight: "800",
                        fontSize: 13,
                      }}
                    >
        ...
        <View style={styles.grid}>
          <StatTile
            label="Ingresos"
            value={formatMoney(month.income, prefs.currency, hide)}
            color={colors.positive}
            icon="arrow-up"
            colors={colors}
            onPress={() => router.push("/insight/income")}
          />
          <StatTile
            label="Gastos"
            value={formatMoney(month.expense, prefs.currency, hide)}
            color={colors.negative}
            icon="arrow-down"
            colors={colors}
            onPress={() => router.push("/insight/expense")}
          />
          <StatTile
            label="Gastos fijos"
          ...
          <StatTile
            ...
            value={formatMoney(month.transfers, prefs.currency, hide)}
            color={colors.transfer}
            icon="swap-horizontal"
            colors={colors}
            onPress={() => router.push("/insight/transfers")}
          />
          <StatTile
            label="Pagado a deudas"
            value={formatMoney(summary.debtPaidThisMonth, prefs.currency, hide)}
            color={colors.transfer}
            icon="trending-down"
            colors={colors}
            onPress={() => router.push("/insight/debt-paid")}
          />
        </View>
...
function TxRow({ t }: { t: Transaction }) {
  ...
  const icon =
    t.type === "transfer"
      ? "swap-horizontal"
      : safeIcon(cat?.icon, isIn ? "arrow-up" : "arrow-down");
  return (
    <Pressable
    ...
    >
      <View
        style={[
          styles.txIcon,
          { backgroundColor: (cat?.color ?? color) + "22" },
        ]}
      >
      ...
      <Text style={{ color, fontWeight: "800" }}>
        {sign}
        {formatMoney(Math.abs(t.amount), prefs.currency, prefs.hideBalances)}
      </Text>
    </Pressable>
  );
}
