...
export default function PeopleScreen() {
  ...
  const borrowed = loans.filter(
    (l) => l.type === "borrowed" && l.personId != null,
  );
  ...
  return (
    <Screen>
      ...
      >
        Personas a quienes les prestas dinero o de quienes recibes. Cada
        préstamo queda asociado a su persona.
      </Text>
      {lent.length === 0 && borrowed.length === 0 ? (
        <EmptyState
          icon="people"
          title="Sin personas"
          subtitle="Crea una persona y regístrala en un préstamo."
        />
      ) : (
        <>
          {lent.length > 0 && (
...
function PersonCard({
  ...
  return (
    <View
      ...
      onTouchEnd={() =>
        loan.personId != null &&
        router.push(`/person/${loan.personId}` as never)
      }
    >
